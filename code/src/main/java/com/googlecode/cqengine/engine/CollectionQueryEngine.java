...
public class CollectionQueryEngine<O> implements QueryEngineInternal<O> {
  ...
  /** {@inheritDoc} */
  // Implementation note: these methods actually just pre-processes QueryOption arguments and then
  // delegates
   ...
   * SimpleQuery}s and the child queries which are {@link CompoundQuery}s. It will call the helper
   * methods above to process the child {@link SimpleQuery}s, and the method will call itself
   * recursively to process the child {@link CompoundQuery}s. Once the method has results for both
   * the child {@link SimpleQuery}s and the child {@link CompoundQuery}s, it will return them in a
   * {@link ResultSetIntersection}, {@link ResultSetUnion} or {@link ResultSetDifference} object as
  ...
  ResultSet<O> retrieveRecursive(Query<O> query, final QueryOptions queryOptions) {
    ...
    if (query instanceof SimpleQuery<O, ?> simpleQuery) {
      ...
      if (DeduplicationOption.isLogicalElimination(queryOptionsForOrUnion)) {
        ...
        union =
            new ResultSetUnion<O>(resultSetsToUnion, query, queryOptions, useIndexMergeStrategy);
      } else {
      ...
      throw new IllegalStateException(
          "Unexpected type of query object: " + getClassNameNullSafe(query));
    }
  }
  ...
  ResultSet<O> retrieveUnionOfSimpleQueries(
    ...
    if (DeduplicationOption.isLogicalElimination(queryOptions)) {
      ...
      boolean useIndexMergeStrategy =
          indexMergeStrategyEnabled && indexesAvailableForAllResultSets(resultSetsToUnion);
